...
The first supported family is C19. The first optimizer source is the COPRO run
already produced by `scripts/run-optim.py`. The command is intentionally a
scriptable CLI rather than a server or interactive TUI. Rich owns terminal
presentation; strict versioned JSON remains the durable debugging boundary.
...
2. `scripts/eval.py` is the user-facing entry point. It uses subcommands so a
   saved run can be inspected without rerunning paid work.
...
- `build_c19_experiment` already maps C19 pools, naive and ceiling probes,
...
```console
uv run --extra optim python scripts/eval.py <command> ...
```

### Explain the task

```console
python scripts/eval.py info c19
python scripts/eval.py info c19 --show-templates
```
...
```console
python scripts/eval.py summary RUN_DIR
python scripts/eval.py failures RUN_DIR
python scripts/eval.py task RUN_DIR TASK_ID
python scripts/eval.py compare RUN_DIR naive ceiling
```
...
```console
python scripts/eval.py trajectory OPTIM_RUN_DIR
python scripts/eval.py trajectory OPTIM_RUN_DIR --show-candidates
```

The default trajectory view shows one ordered row per evaluation resolution,
preserving repeated evaluations rather than silently collapsing them:
...
4. **Observations**: candidate/task/repeat coordinate, rendered prompt, raw
   output, normalized output when present, score, row state, typed failure and
   provider fields, budget fields, submission result, and ordered component
   trace.
...
present because this is a local debugging artifact. API keys, authorization
...
- budget deltas and terminal status; and
...
models. Repeated evidence may reference shared task and candidate records but
keeps its own role, purpose, and resolution coordinate. This makes future
per-task trajectory regressions possible without reopening the SQLite store.
...
command to return nonzero with the projection error.
...
- `scripts/eval.py`: five-line entry point analogous to `run-optim.py`.
...
Use two stacked implementation PRs because trajectory publication depends on
the eval report model, while each layer is independently useful and testable.

### PR 1: C19 eval reports and terminal inspection
...
- Update optimizer CLI documentation and end-to-end fake COPRO coverage.

Do not split Python schema, Rich presentation, and tests into separate PRs;
each behavior lands with its contract and verification. HTML remains a later
optional PR after both JSON contracts have survived real run inspection.
...
inspect one fake eval report and one fake COPRO trajectory at both a narrow and
wide terminal width before relying on layout claims.
