...
The report is a plain scientific debugging instrument, not an experiment
marketing dashboard:

- white canvas, dark neutral text, and light gray structural rules;
- the colorblind-safe Okabe-Ito blue (`#0072B2`) for selection and navigation
  and green (`#009E73`) for successful evidence, with pale tints for local
  emphasis;
- neutral blue-gray treatment for failure and missing states so color does not
  compete with their explicit labels;
...
document respects reduced-motion preferences and intentionally stays in a
stable light theme so screenshots and dense scientific comparisons remain
...
It makes no fetch requests. The implementation adds no speculative report-size
limit, pagination, or split assets.
...
The private HTML renderer under `whetstone_envs.reporting` consumes only the
strict report loaders used by Rich inspection. A repository-owned shell, CSS,
and classic vanilla-JavaScript file are loaded with `importlib.resources` and
embedded with exact CSP hashes. `eval.py html RUN_DIR` and
`eval.py trajectory-html RUN_DIR` print the absolute generated path.

The renderer reuses the C19 info model and Python-owned candidate, accounting,
matrix, and paired-comparison derivations. JavaScript owns inert DOM
construction, interaction, filtering, exact fragment restoration, and
on-demand detail rendering, not evaluation math. Direct pinned-Chromium tests
validate report-to-DOM behavior and compare wide/narrow screenshots with
committed fixtures in CI. The cross-platform comparison uses bounded
block-level color error rather than claiming byte-identical Linux and macOS
rasterization, and a materially different-page check pins its rejection
behavior. The implementation leaves the authoritative report JSON unchanged.
...
- A generic visualization grammar for additional environments.
- MiniGrid animation.
