r"""The two c11 probe prompts, from the baseline spec (Section 2).

Probe (a), the naive prompt, is copied byte-for-byte from the spec's
Section 2.1 ``<pre><code>`` block. Probe (b), the ceiling prompt, is
copied from Section 2.2 -- except its four *worked-example outputs*,
which the spec's Section 7.5 evidence-note flags as hand-typed
placeholders that "must be regenerated by ``rfc8785.dumps`` on the exact
inputs and pasted verbatim before freezing."
...
# The four worked-example *inputs*, verbatim from spec Section 2.2. Their
# Output lines are regenerated below, never hand-typed (spec Section 7.5).
...
# The fixed rule-and-example preamble of the ceiling prompt, verbatim from
# spec Section 2.2 down to (but not including) the worked examples, and the
# fixed tail after them.
