...
def public_prompt_identity(instance: Instance) -> tuple[tuple[str, str], ...]:
    ...
    Public prompt identity is defined solely by ``prompt_inputs``, sorted
    by key. It intentionally excludes private generation and evaluation
    metadata such as ``id``, ``seed``, ``strata``, and ``gold``.
    """
