...
"""
    ...
    assert completed.stdout.strip() == (
        "6f6900adcfbe4fc4c6360c30b9eb09bb5aaece90e568906a2084c39cf71d7a40"
    )
...
def test_manifest_and_regeneration_are_canonical_and_byte_identical(
    ...
    assert first == second == build_manifest(pool)
...
def test_build_manifest_rejects_wrong_or_empty_pool() -> None:
    with pytest.raises(TypeError, match="TaskPool"):
        build_manifest(
...
def test_default_manifest_seed_range_covers_shared_scene_seeds(
    default_pool: TaskPool,
) -> None:
    manifest = build_manifest(default_pool)
